...
<script>
    ...
    confirm_otp.addEventListener('click', function (e) {
        ...
        if (!/^\d{6}$/.test(code)) {
            ...
            model2=document.getElementById('model').value
            ...
            htmx.ajax('POST', '{{route('frontend.confirm.otp')}}', {
                ...
                values: { _token: "{{ csrf_token() }}" ,availability: availability2, code: code, phone_hidden: phone_hidden2 , destination_id: destination_id2, start_id: start_id2, email_hidden: email_hidden2, from: from2, destination: destination2 ,transport_type: transport_type2 , operable: operable2 , year: year2,make: make2 ,model:model2  }
            });
        }
    });
