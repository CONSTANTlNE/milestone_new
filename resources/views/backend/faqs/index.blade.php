...
@section('title') {{ __('admin.sidebar_faqs') }} @endsection
@section('styles')
    ...
    <div class="content">
        <div class="main-content">
            ...
            <div class="block justify-between page-header md:flex">
                <div>
                    <h3 class="!text-defaulttextcolor dark:!text-defaulttextcolor/70 dark:text-white dark:hover:text-white text-[1.375rem] font-semibold font-first-geo">{{ __('admin.all_faqs') }}</h3>
                ...
                <ol class="flex items-center whitespace-nowrap min-w-0 gap-3 header-nav-links">
                    @can('backend.faqs.create')
                    <li class="text-[0.813rem] ps-[0.5rem]">
                        <a href="{{ route('backend.faqs.create') }}" class="ti-btn bg-primary text-white !font-medium font-second-geo">
                            <i class="ri-add-circle-line text-[1.375rem]"></i>
                            {{__('admin.create_new_faqs')}}
                        </a>
                    </li>
                    @endcan
                    ...
                    <div class="box custom-box">
                        <div class="box-header justify-between">
                            <div class="box-title box-show-number gap-5">
                                ...
                                @can('backend.faqs.massDestroy')
                                    <x-backend.table.massDestroy
                                        :url="route('backend.faqs.massDestroy')"
                                    />
                                @endcan
                            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table whitespace-nowrap table-bordered min-w-full" id="datatablesTable">
                                    ...
                                    <tr class="border-b border-primary/10">
                                        ...
                                        <th scope="col" class="service_id text-start">{{ __('admin.sidebar_services') }}</th>
                                    ...
                                    <tbody>
                                    @forelse($faqs as $faq)
                                        <tr class="product-list border-b border-primary/10" data-id="{{$faq->id}}">
                                            @can('backend.pages.massDestroy')
                                            <td class="text-center">
                                                <input class="form-check-input list-checkbox-item cursor-pointer" type="checkbox" value="{{$faq->id}}">
                                            </td>
                                            @endcan
                                            <td>
                                                {{$faq->id}}
                                            ...
                                            <td>
                                                <x-backend.translation-text :model="$faq" field="title" :limit="40" />
                                            </td>
                                            <td>
                                                @if($faq->service)
                                                    <x-backend.translation-text :model="$faq->service" field="title" :limit="20" />
                                                @else
                                                    {{ __('admin.main_faqs')}}
                                                @endif
                                            </td>
                                            <td>
                                                <x-backend.badge type="light" :text="$faq->created_at->format('d/m/Y H:i')" />
                                            </td>
                                            <td>
                                                <x-backend.table.status
                                                :model="$faq"
                                                :statusUrl="route('backend.faqs.status')"
                                                />
                                            </td>
                                            <td>
                                                <x-backend.table.actions
                                                    :model="$faq"
                                                    show-view="backend.faqs.show"
                                                    show-edit="backend.faqs.edit"
                                                    show-delete="backend.faqs.delete"
                                                />
                                            </td>
                                        </tr>
                                    @empty
                                        <tr class="empty-state">
                                            <td colspan="7" class="text-center py-8">
                                                <x-backend.table.empty-state
                                                    :actionText="__('admin.create_first_faqs')"
                                                    permission="backend.faqs.create"
                                                />
                                            </td>
                                        </tr>
                                    @endforelse
                        ...
                        <x-backend.pagination :paginator="$faqs" />
                    </div>
                </div>
            </div>
        </div>
