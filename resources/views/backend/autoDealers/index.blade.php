...
@section('title') {{ __('admin.auto_dealers') }} @endsection
@section('styles')
    ...
    <div class="content">
        <div class="main-content">
            ...
            <div class="block justify-between page-header md:flex">
                <div>
                    <h3 class="!text-defaulttextcolor dark:!text-defaulttextcolor/70 dark:text-white dark:hover:text-white text-[1.375rem] font-semibold font-first-geo">{{ __('admin.all_auto_dealers') }}</h3>
            ...
            <div class="grid grid-cols-12 gap-6 index-table-page white-bg">
                <div class="xl:col-span-12 col-span-12">
                    <div class="box custom-box">
                        <div class="box-header justify-between">
                            <div class="box-title box-show-number gap-5">
                                ...
                                <x-backend.table.massDestroy
                                    :url="route('backend.autoDealers.massDestroy')"
                                />
                                <a href="{{ route('backend.autoDealers.export') }}"
                        ...
                        <div class="box-body">
                            <div class="table-responsive">
                                <table class="table whitespace-nowrap table-bordered min-w-full" id="datatablesTable">
                                    ...
                                    <tbody>
                                    @forelse($autoDealers as $autoDealer)
                                        <tr class="product-list border-b border-primary/10" data-id="{{$autoDealer->id}}">
                                            <td class="text-center">
                                                <input class="form-check-input list-checkbox-item cursor-pointer" type="checkbox" value="{{$autoDealer->id}}">
                                            </td>
                                            <td>
                                                {{$autoDealer->id}}
                                            </td>
                                            <td>
                                                <span class="font-medium">{{$autoDealer->legal_business_name}}</span>
                                                @if($autoDealer->dba)
                                                    <br><small class="text-muted">DBA: {{$autoDealer->dba}}</small>
                                                @endif
                                            </td>
                                            <td>
                                                {{$autoDealer->contact_name}}
                                                <br><small class="text-muted">{{$autoDealer->contact_title}}</small>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <x-backend.badge type="light" :text="$autoDealer->business_type" />
                                            </td>
                                            <td>
                                                <x-backend.badge type="info" :text="$autoDealer->years_operation . ' years'" />
                                            </td>
                                            <td>
                                            ...
                                            <a href="{{ Storage::url($autoDealer->w9_upload) }}" target="_blank" class="ti-btn ti-btn-sm ti-btn-primary">
                                                        ...
                                                        <a href="{{ Storage::url($autoDealer->vehicle_list) }}" target="_blank" class="ti-btn ti-btn-sm ti-btn-warning">
                                                           VL
                                                        </a>
                                                </div>
                                            </td>
                                            <td>
                                                <x-backend.badge type="light" :text="$autoDealer->created_at->format('d/m/Y H:i')" />
                                            </td>
                                            <td>
                                                <x-backend.table.actions
                                                    :model="$autoDealer"
                                                    show-view="backend.autoDealers.show"
                                                    show-delete="backend.autoDealers.delete"
                                                />
                                            </td>
                                        </tr>
                                    @empty
                                        <tr class="empty-state">
                                            <td colspan="12" class="text-center py-8">
                                                {{__('admin.no_auto_dealers_found')}}
                                            </td>
                                        </tr>
                                    @endforelse
                                    </tbody>
                        ...
                        <x-backend.pagination :paginator="$autoDealers" />
                    </div>
                </div>
            </div>
        </div>
