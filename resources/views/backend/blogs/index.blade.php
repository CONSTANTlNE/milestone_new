    ...
    <div class="content">
        <div class="main-content">
            ...
            <div class="block justify-between page-header md:flex">
                <div>
                    <h3 class="!text-defaulttextcolor dark:!text-defaulttextcolor/70 dark:text-white dark:hover:text-white text-[1.375rem] font-semibold font-first-geo">{{ __('admin.all_blogs') }}</h3>
                            ...
                            <div class="box-title box-show-number gap-5">
                                ...
                                @can('backend.blogs.massDestroy')
                                    <x-backend.table.massDestroy
                                        :url="route('backend.blogs.massDestroy')"
                                    />
                                @endcan
                            </div>
                                    ...
                                    @forelse($blogs as $blog)
                                        <tr class="product-list border-b border-primary/10" data-id="{{$blog->id}}">
                                            ...
                                            <td>
                                                ...
                                                :model="$blog"
                                                :statusUrl="route('backend.blogs.status')"
                                                />
                                            </td>
                                            <td>
                                                <x-backend.table.actions
                                                    :model="$blog"
                                                    show-view="backend.blogs.show"
                                                    show-edit="backend.blogs.edit"
                                                    show-delete="backend.blogs.delete"
                                                />
                                            </td>
                                        </tr>
                                    @empty
                                        <tr class="empty-state">
                                            <td colspan="7" class="text-center py-8">
                                                <x-backend.table.empty-state
                                                    :actionText="__('admin.create_first_blog')"
                                                    permission="backend.blogs.create"
                                                />
                                            </td>
                                        </tr>
                                    @endforelse
