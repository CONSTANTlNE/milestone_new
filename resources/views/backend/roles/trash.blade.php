...
@section('title') {{ __('admin.deleted_roles') }} @endsection
@section('styles')
    ...
    <div class="content">
        <div class="main-content">
            ...
            <div class="block justify-between page-header md:flex">
                <div>
                    <h3 class="!text-defaulttextcolor dark:!text-defaulttextcolor/70 dark:text-white dark:hover:text-white text-[1.375rem] font-semibold font-first-geo">{{ __('admin.deleted_roles') }}</h3>
                    ...
                    <div class="box custom-box">
                        <div class="box-header justify-between">
                            <div class="box-title box-show-number gap-5">
                                ...
                                @can('backend.roles.massRemove')
                                    <x-backend.table.massRemove
                                        :url="route('backend.roles.massRemove')"
                                    />
                                @endcan
                            </div>
                                        ...
                                        <tr class="product-list border-b border-primary/10" data-id="{{$role->id}}">
                                            ...
                                            <td>
                                                <x-backend.table.actions
                                                    :model="$role"
                                                    show-remove="backend.roles.remove"
                                                    show-restore="backend.roles.restore"
                                                />
                                            </td>
                                        </tr>
                                    @empty
                                        <tr class="empty-state">
                                            <td colspan="7" class="text-center py-8">
                                                <x-backend.table.empty-state
                                                    :actionText="__('admin.all_roles')"
                                                    :trash="true"
                                                    permission="backend.roles.index"
                                                />
                                            </td>
                                        </tr>
                                    @endforelse
