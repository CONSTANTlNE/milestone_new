...
@section('title') {{ __('admin.view_serviceCategories') }} @endsection
@section('content')
    <div class="content">
            ...
            <div class="block justify-between page-header md:flex">
                <div>
                    <h3 class="!text-defaulttextcolor dark:!text-defaulttextcolor/70 dark:text-white dark:hover:text-white text-[1.375rem] font-semibold font-first-geo">
                        {{ __('admin.view_serviceCategories') }}
                    </h3>
                ...
                <ol class="flex items-center whitespace-nowrap min-w-0 gap-3 header-nav-links">
                    ...
                    @endcan
                    @can('backend.serviceCategories.update')
                        <li class="text-[0.813rem] ps-[0.5rem]">
                            <a href="{{ route('backend.serviceCategories.edit', $serviceCategory->id) }}" class="ti-btn bg-warning text-white !font-medium font-second-geo">
                                <i class="ri-edit-line text-[1.375rem]"></i>
                                {{__('admin.edit_serviceCategories')}}
                            </a>
                        </li>
                    @endcan
                            ...
                            <div class="flex items-center space-x-4 p-4 bg-primary/5 rounded-lg">
                                ...
                                <div class="mx-3">
                                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white">
                                        {{ $serviceCategory->getTranslation('title', app()->getLocale()) ?: $serviceCategory->getTranslation('title', 'en') }}
                                    </h4>
                                    <p class="text-sm text-gray-500 font-second-geo">
                                        {{ __('admin.serviceCategory_id') }}: #{{ $serviceCategory->id }} | {{ __('admin.status') }}
                                        <span class="text-white badge {{ $serviceCategory->status ? 'bg-success' : 'bg-secondary' }}">
                                            {{ $serviceCategory->status ? __('admin.active') : __('admin.inactive') }}
                                        </span>
                                    </p>
                                    <hr>
                                </div>
                        ...
                        <div class="box-body">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                                    ...
                                    <p class="card-text">
                                        <x-backend.imageDefault :src="$serviceCategory->src" />
                                    </p>
                                </div>
                            </div>
                        </div>
