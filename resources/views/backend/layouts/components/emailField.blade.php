...
<div class="col-md-{{$width}}">
    <div class="form-group position-relative">
        <label class="font-w600" @if($required) for="validationTooltip{{$columnId}}" @endif>{{ __('strings.'.$staticKeyLabel)  }}</label>
        <input type="email" class="form-control" name="{{$title}}" value="{{$value}}" placeholder="" @if($required)  id="validationTooltip{{$columnId}}"  required @endif>
        @if($helpText && $successText)
        <div class="valid-tooltip">
            {{ __('strings.'.$staticKeySuccessText) }}
        </div>
        <div class="invalid-tooltip">
            {{ __('strings.'.$staticKeyhelpText) }}
        </div>
        @endif
        @include('backend.layouts.components.error', ['email' => $title])
    </div>
</div>
