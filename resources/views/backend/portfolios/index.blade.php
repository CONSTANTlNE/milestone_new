...
@section('title') {{ __('admin.sidebar_portfolios') }} @endsection
@section('styles')
    ...
    <div class="content">
        <div class="main-content">
            ...
            <div class="block justify-between page-header md:flex">
                <div>
                    <h3 class="!text-defaulttextcolor dark:!text-defaulttextcolor/70 dark:text-white dark:hover:text-white text-[1.375rem] font-semibold font-first-geo">{{ __('admin.all_portfolios') }}</h3>
                ...
                <ol class="flex items-center whitespace-nowrap min-w-0 gap-3 header-nav-links">
                    @can('backend.portfolios.create')
                    <li class="text-[0.813rem] ps-[0.5rem]">
                        <a href="{{ route('backend.portfolios.create') }}" class="ti-btn bg-primary text-white !font-medium font-second-geo">
                            <i class="ri-add-circle-line text-[1.375rem]"></i>
                            {{__('admin.create_new_portfolio')}}
                        </a>
                    </li>
                    @endcan
                    ...
                    <div class="box custom-box">
                        <div class="box-header justify-between">
                            <div class="box-title box-show-number gap-5">
                                ...
                                @can('backend.portfolios.massDestroy')
                                    <x-backend.table.massDestroy
                                        :url="route('backend.portfolios.massDestroy')"
                                    />
                                @endcan
                            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table whitespace-nowrap table-bordered min-w-full" id="datatablesTable">
                                    ...
                                    @forelse($portfolios as $portfolio)
                                        <tr class="product-list border-b border-primary/10" data-id="{{$portfolio->id}}">
                                            @can('backend.sliders.massDestroy')
                                            <td class="text-center">
                                                <input class="form-check-input list-checkbox-item cursor-pointer" type="checkbox" value="{{$portfolio->id}}">
                                            </td>
                                            @endcan
                                            <td>
                                                {{$portfolio->id}}
                                            ...
                                            <td>
                                                <x-backend.translation-text :model="$portfolio" field="title" :limit="40" />
                                            </td>
                                            <td class="flex justify-center">
                                                <x-backend.image :src="$portfolio->src" />
                                            </td>
                                            <td>
                                                <x-backend.badge type="light" :text="$portfolio->created_at->format('d/m/Y H:i')" />
                                            </td>
                                            <td>
                                                <x-backend.table.status
                                                :model="$portfolio"
                                                :statusUrl="route('backend.portfolios.status')"
                                                />
                                            </td>
                                            <td>
                                                <x-backend.table.actions
                                                    ...
                                                    show-edit="backend.portfolios.edit"
                                                    show-delete="backend.portfolios.delete"
                                                />
                                            </td>
                                        </tr>
                                    @empty
                                        <tr class="empty-state">
                                            <td colspan="7" class="text-center py-8">
                                                <x-backend.table.empty-state
                                                    :actionText="__('admin.create_first_portfolio')"
                                                    permission="backend.portfolios.create"
                                                />
                                            </td>
                                        </tr>
                                    @endforelse
                        ...
                        <x-backend.pagination :paginator="$portfolios" />
                    </div>
                </div>
            </div>
        </div>
