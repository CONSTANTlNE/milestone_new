...
@section('title') {{ __('strings.View Article') }} - Versus @endsection
@section('content')
    <div class="container-fluid">
        <div class="form-head d-md-flex mb-sm-4 mb-3 align-items-start">
            <div class="mr-auto  d-lg-block">
                <h2 class="text-black font-w600">{{ __('strings.View Article') }} - Versus</h2>
            ...
            @can('backend.versus.create')
                <a href="{{ route('backend.versus.create', app()->getLocale())}}" class="btn btn-primary rounded ml-3"><i class="flaticon-381-add-2"></i> {{ __('strings.Add a new Versus') }}</a>
            @endcan
                    ...
                    <div class="card-body">
                        <div class="bg-primary text-white-50">
                            <div class="card-body" style="color:#fff;">
                                ...
                                <h5 class="mt-0 mb-4 text-white"><i class="mdi mdi-bullseye-arrow"></i>
                                    {{ __('strings.Title1') }} -
                                    @if (Arr::get($article->versus->first()->getTranslations('title1'), app()->getLocale()) === null)
                                        {{ __('strings.TranslationTitle') }} ({{ __('strings.Translated') }} {{ implode(', ', array_keys($article->versus->first()->getTranslations('title1'))) }})
                                    @else
                                        {{Str::limit($article->versus->first()->getTranslation('title1', app()->getLocale()), 255)}}
                                    @endif
                                </h5>
                                <hr>
                                <h5 class="mt-0 mb-4 text-white"><i class="mdi mdi-bullseye-arrow"></i>
                                    {{ __('strings.Title2') }} -
                                    @if (Arr::get($article->versus->first()->getTranslations('title2'), app()->getLocale()) === null)
                                        {{ __('strings.TranslationTitle') }} ({{ __('strings.Translated') }} {{ implode(', ', array_keys($article->versus->first()->getTranslations('title2'))) }})
                                    @else
                                        {{Str::limit($article->versus->first()->getTranslation('title2', app()->getLocale()), 255)}}
                                    @endif
                                </h5>
                                <hr>
                                ...
                                @if(!empty($article->versus->first()->getTranslations('slogan1')))
                                    <hr>
                                    <p class="card-text">{{ __('strings.Slogan1') }} -
                                        @if (Arr::get($article->versus->first()->getTranslations('slogan1'), app()->getLocale()) === null)
                                            {{ __('strings.Translated') }} {{ implode(', ', array_keys($article->versus->first()->getTranslations('slogan1'))) }}
                                        @else
                                            {{ $article->versus->first()->getTranslation('slogan1', app()->getLocale()) }}
                                        @endif
                                    </p>
                                @endif

                                @if(!empty($article->versus->first()->getTranslations('slogan2')))
                                    <hr>
                                    <p class="card-text">{{ __('strings.Slogan2') }} -
                                        @if (Arr::get($article->versus->first()->getTranslations('slogan2'), app()->getLocale()) === null)
                                            {{ __('strings.Translated') }} {{ implode(', ', array_keys($article->versus->first()->getTranslations('slogan2'))) }}
                                        @else
                                            {{ $article->versus->first()->getTranslation('slogan2', app()->getLocale()) }}
                                        @endif
                                    </p>
                                @endif
                                ...
                                <hr>
                                <p class="card-text">ვერდიქტი2 -
                                    @if(isset($article->verdict_versus_id))
                                        {{$article->verdictVersus()->first()->getTranslation('title', app()->getLocale()) ?? "ვერდიქტი გარეშე"}}
                                    @else
                                        ვერდიქტი გარეშე
                                    @endif
                                </p>
