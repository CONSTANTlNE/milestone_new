...
@section('title') {{ __('admin.view_slider') }} @endsection
@section('content')
    <div class="content">
                ...
                <ol class="flex items-center whitespace-nowrap min-w-0 gap-3 header-nav-links">
                    @can('backend.sliders.index')
                        <li class="text-[0.813rem] ps-[0.5rem]">
                            <a href="{{ route('backend.sliders.index') }}" class="ti-btn bg-secondary text-white !font-medium font-second-geo">
                                <i class="ri-arrow-go-back-line text-[1.375rem]"></i>
                                {{ __('admin.return_back') }} - {{ __('admin.all_sliders') }}
                            </a>
                        </li>
                    @endcan
                    @can('backend.sliders.update')
                        <li class="text-[0.813rem] ps-[0.5rem]">
                            <a href="{{ route('backend.sliders.edit', $slider->id) }}" class="ti-btn bg-warning text-white !font-medium font-second-geo">
                                <i class="ri-edit-line text-[1.375rem]"></i>
                                {{__('admin.edit_sliders')}}
                            </a>
                        </li>
                    @endcan
                    ...
                    <div class="box">
                        <div class="box-body">
                            <div class="flex items-center space-x-4 p-4 bg-primary/5 rounded-lg">
                                ...
                                <div class="mx-3">
                                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white">
                                        {{ $slider->getTranslation('title', app()->getLocale()) ?: $slider->getTranslation('title', 'en') }}
                                    </h4>
                                    <p class="text-sm text-gray-500 font-second-geo">
                                        {{ __('admin.slider_id') }}: #{{ $slider->id }} | {{ __('admin.status') }}
                                        <span class="text-white badge {{ $slider->status ? 'bg-success' : 'bg-secondary' }}">
                            ...
                            <div class="flex items-center space-x-4 p-4 bg-primary/5 rounded-lg mt-4">
                                <p class="text-sm text-gray-500 font-second-geo">
                                    {{ __('admin.short_description') }}: {{ $slider->slogan }}
                                </p>
                            </div>

                            <div class="flex items-center space-x-4 p-4 bg-primary/5 rounded-lg mt-4">
                                <p class="text-sm text-gray-500 font-second-geo">
                                    {{ __('admin.url') }}: {{ $slider->url }}
                                </p>
                            </div>
                        ...
                        <div class="box-body">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                                    ...
                                    <p class="card-text">
                                        <x-backend.imageDefault :src="$slider->src" />
                                    </p>
                                </div>
                            </div>
                        </div>
