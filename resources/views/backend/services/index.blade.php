...
@section('title') {{ __('admin.sidebar_services') }} @endsection
@section('styles')
    ...
    <div class="content">
        <div class="main-content">
            ...
            <div class="block justify-between page-header md:flex">
                <div>
                    <h3 class="!text-defaulttextcolor dark:!text-defaulttextcolor/70 dark:text-white dark:hover:text-white text-[1.375rem] font-semibold font-first-geo">{{ __('admin.all_services') }}</h3>
                            ...
                            <div class="box-title box-show-number gap-5">
                                ...
                                @can('backend.services.massDestroy')
                                    <x-backend.table.massDestroy
                                        :url="route('backend.services.massDestroy')"
                                    />
                                @endcan
                            </div>
                                    ...
                                    @forelse($services as $service)
                                        <tr class="product-list border-b border-primary/10" data-id="{{$service->id}}">
                                            ...
                                            <td class="flex justify-center">
                                                <x-backend.image :src="$service->src" />
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                :model="$service"
                                                :statusUrl="route('backend.services.status')"
                                                />
                                            </td>
                                            <td>
                                                <x-backend.table.actions
                                                    :model="$service"
                                                    show-view="backend.services.show"
                                                    show-edit="backend.services.edit"
                                                    show-delete="backend.services.delete"
                                                />
                                            </td>
                                        </tr>
                                    @empty
                                        <tr class="empty-state">
                                            <td colspan="7" class="text-center py-8">
                                                <x-backend.table.empty-state
                                                    :actionText="__('admin.create_first_service')"
                                                    permission="backend.services.create"
                                                />
                                            </td>
                                        </tr>
                                    @endforelse
