...
@section('title') {{ __('admin.view_blogCategories') }} @endsection
@section('content')
    <div class="content">
        <div class="main-content">
            <div class="block justify-between page-header md:flex">
                ...
                <ol class="flex items-center whitespace-nowrap min-w-0 gap-3 header-nav-links">
                    ...
                    @endcan
                    @can('backend.blogCategories.update')
                        <li class="text-[0.813rem] ps-[0.5rem]">
                            <a href="{{ route('backend.blogCategories.edit', $blogCategory->id) }}" class="ti-btn bg-warning text-white !font-medium font-second-geo">
                                <i class="ri-edit-line text-[1.375rem]"></i>
                                {{__('admin.edit_blogCategories')}}
                            </a>
                        </li>
                    @endcan
                            ...
                            <div class="flex items-center space-x-4 p-4 bg-primary/5 rounded-lg">
                                ...
                                <div class="mx-3">
                                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white">
                                        {{ $blogCategory->getTranslation('title', app()->getLocale()) ?: $blogCategory->getTranslation('title', 'en') }}
                                    </h4>
                                    <p class="text-sm text-gray-500 font-second-geo">
                                        {{ __('admin.blogCategory_id') }}: #{{ $blogCategory->id }} | {{ __('admin.status') }}
                                        <span class="text-white badge {{ $blogCategory->status ? 'bg-success' : 'bg-secondary' }}">
                                            {{ $blogCategory->status ? __('admin.active') : __('admin.inactive') }}
                                        </span>
                                    </p>
                                    <hr>
                                </div>
                        ...
                        <div class="box-body">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                                    ...
                                    <p class="card-text">
                                        <x-backend.imageDefault :src="$blogCategory->src" />
                                    </p>
                                </div>
                            </div>
                        </div>
