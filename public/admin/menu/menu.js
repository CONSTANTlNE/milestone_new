...
function getmenus() {
    ...
    $('#menu-to-edit li').each(function(index) {
        ...
        // Correctly calculate the depth from the class
        for (var i = 0; i <= 10; i++) {
        ...
        // Assign the correct parent ID based on the hierarchy
        if (dept > 0) {
            var parentElement = $(this).prevAll(`.menu-item-depth-${dept - 1}`).first();
            if (parentElement.length > 0) {
                padre = parentElement.attr('id').split('-')[2];
            }
        }
...
function addcustommenu(element) {
    $('#spincustomu').show();
    let $prefix = $(element).closest('.customlinkdiv').find('#custom-menu-item-prefix').val();
    let $slug = $(element).closest('.customlinkdiv').find('#custom-menu-item-slug').val();
    let $title = $(element).closest('.customlinkdiv').find('#custom-menu-item-title-' + $prefix).val();
    let $model = $(element).closest('.customlinkdiv').find('#custom-menu-item-model').val();
