...
//Case Changing
  ...
  if ((markup == 'semantic') || (markup == 'xhtml') || (markup == 'css')) {
    html = html.replace(new RegExp('<font([^>]*)face="([^>]*)">(.*?)<\/font>', 'gi'), '$3');
...
function pre_filter_linebreaks(html, markup) {
  if (browser() == 'webkit') {
    html = html.replace(/<br class="khtml-block-placeholder">/gi, '');
    html = html.replace(/<br class="webkit-block-placeholder">/gi, '');
  }
  html = html.replace(/<br>/gi, '');
  html = html.replace(/<br (.*?)>/gi, '');
  html = html.replace(/<br\/>/gi, '');
  html = html.replace(/<br \/>/gi, '');
  html = html.replace(/<div><NOTE_BR><\/div>/gi, '');
