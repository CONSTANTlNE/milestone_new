...
class TeamsDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('title', function ($row) {
                if (Arr::get($row->getTranslations('title'), app()->getLocale()) === null) {
                    ...
                    if (count($keys) > 0){
                        ...
                        return __('strings.TranslationTitle');
                    }
                } else {
            ...
            ->editColumn('work', function ($row) {
                if (Arr::get($row->getTranslations('work'), app()->getLocale()) === null) {
                    $keys = array_keys($row->getTranslations('work'));
                    if (count($keys) > 0){
                        return __('strings.TranslationTitleWork'). ' ('. __('strings.Translated') . implode(', ', $keys). ')';
                    }else{
                        return __('strings.TranslationTitleWork');
                    }
                } else {
                    return Str::limit($row->getTranslation('work', app()->getLocale()), 30);
                }
            })
            ...
            ->editColumn('actions', function ($row) {
                $statusGate    = 'backend.teams.status';
                $showGate      = 'backend.teams.show';
                $editGate      = 'backend.teams.edit';
                $destroyGate   = 'backend.teams.destroy';
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make(['data' => 'work', 'title' => __('strings.Work')], 'work')->orderable(false),
