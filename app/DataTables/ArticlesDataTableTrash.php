...
class ArticlesDataTableTrash extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('actions', function ($row) {
                $restoreGate      = 'backend.articles.restore';
                $removeGate    = 'backend.articles.remove';
    ...
    {
        return $model->onlyTrashed()->newQuery()->where('type', 1);
    }
    ...
    {
        return 'ArticlesTrash_'.date('YmdHis');
    }
}
